...
def f(x):
    return 8*x**(21) + 5*x**(5) - math.sqrt(x)

first_deriv = float(sp.diff(f(x), x))
second_deriv = float(sp.diff(f(x), x, x))
third_deriv = float(sp.diff(f(x), x, x, x))
